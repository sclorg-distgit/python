...
Source8: pythondeps-scl-34.sh

# Append 33 for the same reason here
Source9: brp-python-bytecompile-with-scl-python-34
...
# Already in upstream since 3.4.0
...
# Upstream since 3.4 http://bugs.python.org/issue19447
...
%if 0%{with_rewheel}
Patch190: 00190-add-rewheel-module.patch
%endif

# Tests requiring SIGHUP to work don't work in Koji                                                 
# see rhbz#1088233                                                                                  
Patch194: 00194-temporarily-disable-tests-requiring-SIGHUP.patch

# Fix test_gdb failure on ppc64le - RHSCL is only on x86_64
#Patch196 00196-test-gdb-match-addr-before-builtin.patch 

# OpenSSL in RHEL has SSL3 enabled
...
# caught by rpmdiff
# In upstream already: http://bugs.python.org/issue18028
Patch200:00200-fix-ceval-type-punned-pointer.patch

# The xmlrpc client library is the only stdlib module that has a gzip decompression 
# handler for compressed HTTP streams. The gzip_decode() function decompresses HTTP 
# bodies that are compressed and sent with Accept-Encoding: x-gzip.
...
# In upsream already: http://bugs.python.org/issue16043
Patch201:00201-xmlrpc-unrestricted-decompression-gzip.patch

# This patch combines two patches from two related issues
# http://bugs.python.org/issue22366
# http://bugs.python.org/issue22417
# Resolves: rhbz#1187795
Patch202:00202-enable-cert-verify-by-default.patch

# If tempfile.NamedTemporaryFile is used as iterator, it can close
# the iterated file before it's actually read
# http://bugs.python.org/issue23700
# This causes urlopen() to fail with ftp url, if used as iterator
...
#%patch183 -p1 upstream in Python 3.4
%patch184 -p1
#%patch185 -p1
%patch186 -p0
#%patch187 -p1 rhel7 only
#%patch188 -p1
#%patch189 -p1
%if 0%{with_rewheel}
%patch190 -p1
...
%endif
  ...
  #
  # Invoke the build:
  make EXTRA_CFLAGS="$CFLAGS $MoreCFlags"
...
echo '[ $? -eq 127 ] && echo "Could not find python%{LDVERSION_optimized}-`uname -m`-config. Look ar    ound to see available arches." >&2' >> \
  %{buildroot}%{_bindir}/python%{LDVERSION_optimized}-config
...
CheckPython() {
  ...
  # test_distutils exclusion: http://bugs.python.org/issue22904#msg239135
  # test_readline exclusion: http://bugs.python.org/issue19884
...
%{?scl:scl enable %scl - << \EOF}
#https://github.com/fedora-python/python3/issues/2
set -e
...
%files devel
%defattr(-,root,root)
%{?scl:%{_root_prefix}/lib/rpm/pythondeps-scl-34.sh}
%{?scl:%{_root_prefix}/lib/rpm/redhat/brp-python-bytecompile-with-scl-python-34}
...
- Fix tempfile.NamedTemporaryFile being closed while iterating
Resolves: rhbz#1203236

* Tue Feb 03 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-8
- Add patch for unrestricted decompression in XMLRPC CVE-2013-1753 
- Add patch to enable HTTPS verify by default CVE-2014-9365
- Enable set -e inside scl enable while calling tests
- Skip failing test_readline test
Resolves: rhbz#1187793 
Resolves: rhbz#1187795

* Thu Jan 29 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-7
- Add patch for fixing type-punned pointer in ceval.c
- Add unversion 2to3

* Wed Jan 21 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-6
- Build with rewheel set to 1

* Tue Jan 20 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-5
- Remove expat version requirement

* Mon Jan 19 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-4
- Change file permissions for executables

* Fri Jan 16 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-3
- Bump for rebuild with newer buildroot

* Fri Jan 16 2015 Robert Kuska <rkuska@redhat.com> - 3.4.2-2
- Fix macros names in devel subpackage so they don't contain dashes

* Wed Jan 14 2015 Matej Stuchlik <mstuchli@redhat.com> - 3.4.2-1
- Updated to 3.4.2
- Added scl-utils-build to Requires (rhbz#1144601)
- Add pynche executable (rhbz#1094640)

* Thu Mar 20 2014 Matej Stuchlik <mstuchli@redhat.com> - 3.3.2-14
- Create a python manpage
Resolves: rhbz#1072522

* Tue Dec 03 2013 Bohuslav Kabrda <bkabrda@redhat.com> - 3.3.2-13
- Rebuild with proper OpenSSL.
Resolves: rhbz#1037513

* Mon Nov 25 2013 Robert Kuska <rkuska@redhat.com> - 3.3.2-12
- Move unversioned python macros to metapackage

* Sun Nov 24 2013 Robert Kuska <rkuska@redhat.com> - 3.3.2-11
...
* Fri Nov 01 2013 Bohuslav Kabrda <bkabrda@redhat.com> - 3.3.2-10
- Make building depending collections on top of python33 easier.
Resolves: rhbz#994570

* Fri Oct 25 2013 Bohuslav Kabrda <bkabrda@redhat.com> - 3.3.2-9
- Apply the patch from previous commit.

* Fri Oct 25 2013 Bohuslav Kabrda <bkabrda@redhat.com> - 3.3.2-8
